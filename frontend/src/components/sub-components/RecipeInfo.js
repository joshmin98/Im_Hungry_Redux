...
class RecipeInfo extends React.Component {
  state = {
    name: 'Burger',
    img:
      'https://www.epicurus.com/food/recipes/wp-content/uploads/2011/09/bbq-cheeseburger.jpg',
    preptime: '10 mins',
    cookTime: '20 mins',
  ...
  componentDidMount() {
    ...
    this.setState({
      name: info.title,
      img: info.image,
      preptime: info.preparationminutes
        ? info.preparationMinutes.toString() + ' mins'
        : null,
      cookTime: info.cookingMinutes
        ? info.cookingMinutes.toString() + ' mins'
        : null,
    ...
    return (
      <div className={classes.root}>
        ...
        <p id="prepTime">
          ...
          {this.state.preptime ? <>{this.state.preptime}</> : <>N/A</>}
        </p>
        <p id="cookTime">
          <strong>Cook time: </strong>
          {this.state.cookTime ? <>{this.state.cookTime}</> : <>N/A</>}
        </p>
        <p>
          ...
          <Grid item xs={6}>
            <ul id="ingredientRight">
              {this.state.Ingredient.map((e, i) => {
                if (i % 2 !== 0) {
                  return (
                    <li className={classes.textRight} key={e}>
                      {e}
                    </li>
                  );
                } else {
