...
const checkButton = async (id,buttonName) => {
  ...
  // })
  const data = await id;
  const text = await scope.context.currentPage.evaluate(data => document.getElementById(data), data);
...
const CheckRestaurantContent = async () => {
  ...
  let phone = await scope.context.currentPage.evaluate(() => {
    return document.getElementsByClassName('restaurantPhoneNumber')[0].innerText;
  })
  assert(name !== '' || drivingTime !== '' || address !== '' || phone !== '', 'Missing Info');
}
...
const checkRecipePage = async (name) => {
  let data = await scope.context.currentPage.evaluate(() => {
    return document.getElementById('Halloumi aubergine burgers with harissa relish').innerText;
  })
  assert(data === name, 'Incorrect Restaurant');
}
